...
test('profile-based deep rewrite changes exported content without changing master facts',async({page})=>{
 ...
 await page.locator('#allowProfileRewrite').check();await page.locator('#rewriteProfileVersion').click();await expect(page.locator('#profileRewriteStatus')).toContainText('正文已更新');await expect(page.locator('#resumePreview')).toContainText('具备资料整理与实验室物料采购经验。');
